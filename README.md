...
一个完全离线运行的个人日语口语练习工具。当前版本支持文本输入、离线语音识别、本地 LLM 回复和系统 TTS 播放日语回应。
...
- 收敛采样参数，并对模型输出做四段学习格式化兜底
- Vosk 离线语音识别入口，可将识别结果回填到输入框
- Android System TTS 播放 AI 的日语自然回应
- 聊天历史清空
...
当前使用 Android System TTS 播放 AI 回复中的 `【自然回应】` 日语内容。若设备提示不支持日语语音，请在系统 TTS 设置中安装日语语音数据。
